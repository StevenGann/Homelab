...
# migrate-qbittorrent2-nfs-to-localpath.sh — copy qBittorrent2 config from Akasha
# NFS to node-local local-path PVC to resolve the D-state hang incident
# (2026-07-17).
#
# qbittorrent-nox was D-sl (uninterruptible sleep, blocked on NFS I/O) on a hard
# NFS mount → port 8085 never binds → startup probe fails → endless restarts.
# The config dataset (BT_backup with 1400+ torrents + ~38MB state) hits NFS hard
# mount stalls. Fix: move to local-path, same as sonarr/radarr/lidarr/prowlarr
# (ADR-0003 interim architecture).
#
# Usage:
#   ./migrate-qbittorrent2-nfs-to-localpath.sh [--force]
#
# Options:
...
NAMESPACE="media"
DEPLOYMENT="qbittorrent2"
APP_LABEL="app=qbittorrent2"
PVC_NAME="qbittorrent2-config"           # same name — downstream (deployment) needs no changes
OLD_PV="akasha-app-media-qbittorrent2-config"
NFS_SERVER="192.168.10.247"
NFS_PATH="/mnt/Media-Storage/Application-Storage/media/qbittorrent2-config"
COPY_JOB="qbittorrent2-nfs-migrate"
TEMP_PV="qbittorrent2-nfs-migrate-pv"
TEMP_PVC="qbittorrent2-nfs-migrate-pvc"
PVC_SIZE="5Gi"                            # 38 MB now, headroom for more torrents
...
log "=== qBittorrent2 NFS -> local-path migration ==="
...
k wait --for=delete pod -l "$APP_LABEL" -n "$NAMESPACE" --timeout=60s 2>/dev/null || {
  warn "Pod did not terminate gracefully (D-state NFS hang likely); force-deleting..."
      ...
      command:
        ...
        - |
          ...
          if [ "\$(ls -A /src/ 2>/dev/null)" ]; then
            ...
            echo "Copy complete."
            echo ""
            echo "Source file count: \$(find /src -type f 2>/dev/null | wc -l)"
            echo "Dest   file count: \$(find /dest -type f 2>/dev/null | wc -l)"
...
log "Waiting for copy job to complete (timeout 600s)..."
if k wait --for=condition=complete "pod/$COPY_JOB" -n "$NAMESPACE" --timeout=600s 2>/dev/null; then
...
ENDPOINTS=$(k get endpoints -n "$NAMESPACE" qbittorrent2 -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null || echo "")
...
log "Endpoints: $(k get endpoints -n "$NAMESPACE" qbittorrent2 -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null || echo '<none yet>')"
log "Service:   http://192.168.10.83:8085"
log ""
