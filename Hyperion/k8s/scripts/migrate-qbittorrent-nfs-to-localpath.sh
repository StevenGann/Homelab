...
# migrate-qbittorrent-nfs-to-localpath.sh — copy qBittorrent (instance A) config
# from Akasha NFS to node-local local-path PVC to resolve the HTTP server hang
# incident (2026-07-17).
#
...
# + restart loop.  Cross-node correlation with qbittorrent3 (also NFS) and
# qbittorrent2 (local-path, clean control) confirms NFS is the root cause.
#
# Usage:
#   ./migrate-qbittorrent-nfs-to-localpath.sh [--force]
#
# Options:
...
APP_LABEL="app=qbittorrent"
PVC_NAME="qbittorrent-config"             # same name — downstream (deployment) needs no changes
OLD_PV="akasha-app-media-qbittorrent-config"
NFS_SERVER="192.168.10.247"
NFS_PATH="/mnt/Media-Storage/Application-Storage/media/qbittorrent-config"
COPY_JOB="qbittorrent-nfs-migrate"
TEMP_PV="qbittorrent-nfs-migrate-pv"
TEMP_PVC="qbittorrent-nfs-migrate-pvc"
PVC_SIZE="5Gi"
...
log "=== qBittorrent (A) NFS -> local-path migration ==="
...
ENDPOINTS=$(k get endpoints -n "$NAMESPACE" qbittorrent -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null || echo "")
...
log "Endpoints: $(k get endpoints -n "$NAMESPACE" qbittorrent -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null || echo '<none yet>')"
log "Service:   http://192.168.10.58:8085"
log ""
