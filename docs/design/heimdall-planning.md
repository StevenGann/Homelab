...
- NGINX (already used on Monolith for the image server; familiar)
...
- Docker Compose stack (matches Monolith style — services live in containers, host stays minimal)
...
- **Observability.** Does Heimdall stream to `journal-remote` on Monolith (consistent with Hyperion), or run its own logging?
